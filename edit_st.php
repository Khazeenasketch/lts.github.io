...
$emp_id= isset($_GET['emp_id']) ? $_GET['emp_id']:'' ;// get id through query string


$qry = mysqli_query($conn,"select * from employe where emp_id='$emp_id'"); // select query
...
if(isset($_POST['update'])) // when click on Update button
{
    ...
    $password = $_POST["st_passwords"];
    $phone=$_POST["st_phone_no"];
    $CNIC= $_POST["st_cnic"];
    ...
    $edit = "UPDATE employe set names='$name', emp_id='$emp_id',br_code='$br_code',email='$email',passwords='$password',phone='$phone',CNIC='$CNIC',, adress='$address',gender='$gender',salary='$salary',dob='$dob', dp_id='$courier'  WHERE br_code=$br_code";
	
    if($edit){
        echo '<script>alert("Branch UPDATED")</script>';
...
-->
        ...
        <h1 style="font-weight:normal;">Update Staff</h1>
           <hr style="border-color:rgba(0,123,255,255);">
           
<br>

<form action="edit_st.php"   method="post"  style="background-color:white; margin: 25px 25px; border:1px solid grey; box-shadow:10px 10px 10px 10px grey;" >
                 <br>
...
$conn->close();
                ...
                <br>
                <input type="text" value="<?php echo $row['email']; ?>"  name="st_email"  style="margin:10px 18px 10px 10px; height:40px; width:47%" class="inputs">
                <input type="password"  value="<?php echo $row['passwords'];?>" name="st_password" style="margin:10px 10px 10px 11px; height:40px; width:47%" class="inputs">
                <br>
                ...
                <br>
                <input type="number"  value="<?php echo $row['phone_no'];?>"name="st_phone"  style="margin:10px 18px 10px 10px; height:40px; width:47%" class="inputs">
                <input type="number" value="<?php echo $row['CNIC'];?>"name="st_cnic"  style="margin:10px 10px 10px 11px; height:40px; width:47%" class="inputs">
                <br>
                ...
                <br>
                <input type="text"value="<?php echo $row['adress'];?>" name="st_address"style="margin:10px 18px 10px 10px;height:40px; width:47%" class="inputs">
                <SELEct value="<?php echo $row['gender'];?>" name="st_gender" style="margin:10px 18px 10px 10px; height:40px; width:47%">
 ...
 <br>
 <input type="number" value="<?php echo $row['salary'];?>" name="st_salary"  style="margin:10px 18px 10px 10px; height:40px; width:47%" class="inputs">
 <input type="date" value="<?php echo $row['dob'];?>" name="st_dob" style="margin:10px 10px 10px 12px; height:40px; width:47%" class="inputs">
    
              <br>
              <label   style="margin:10px 10px 20px 10px;" > <b>Courier</b> </label>
                <br>
               
                <select value="<?php echo $row['dp_id'];?>" name="dp_id" style="margin:10px 18px 10px 10px; height:40px; width:47%" >
                  <?php
                        $conn = mysqli_connect("localhost", "root", "", "lts_db");
// Check connection
if ($conn->connect_error) {
...
$sql_s = "SELECT dp_id FROM courier";
