...
$br_code= isset($_GET['br_code']) ? $_GET['br_code']:'' ;// get id through query string


$qry = mysqli_query($conn,"select * from branches where br_code='$br_code'"); // select query
...
if(isset($_POST['update'])) // when click on Update button
{
    ...
    $br_code= isset($_POST['br_code']) ? $_POST['br_code']: '';
    $br_manager = $_POST["br_manager"];
    $address = $_POST["address"];
    $city = $_POST["city"];
    $contact=$_POST["contact"];

    $edit = "UPDATE branches set adress='$address', city='$city',br_manager='$br_manager',br_name='$br_name',city='$city',contact='$contact'WHERE br_code=$br_code";
...
-->
        ...
        <h1 style="font-weight:normal;">Update Branch</h1>
           <hr style="border-color:rgba(0,123,255,255);">
           
<br>

<form action="edit_br.php"   method="post"  style="background-color:white; margin: 25px 25px; border:1px solid grey; box-shadow:10px 10px 10px 10px grey;" >
                 <br>
                <label for="labels" style="margin:10px 10px 10px 10px;"> <b> Address</b></label>
                <label style="margin:10px 10px 20px 455px;"> <b>City</b> </label>
                <br>
                <input type="text" name="address" value="<?php echo $row["adress"];?>" style="margin:10px 18px 10px 10px; height:40px; width:47%" class="inputs">
                <input type="text" name="city" value="<?php echo $row["city"];?>" style="margin:10px 10px 10px 10px; height: 40px; width:47%" class="inputs">
                <br>
 
                <label for="labels"  style="margin:10px 10px 10px 10px;"> <b> Branch Manager</b></label>
                <label style="margin:10px 10px 20px 395px;"> <b>Branch Name</b> </label>
                <br>
                <input type="text"name="br_manager" value="<?php echo $row["br_manager"];?>" style="margin:10px 18px 10px 10px; height:40px; width:47%" class="inputs">
                <input type="text" name="br_name" method='get'  value="<?php echo$row["br_name"];?>" style="margin:10px 10px 10px 10px; height: 40px; width:47%" class="inputs">
                <br>


                <label style="margin:10px 10px 10px 10px;"> <b>Contact#</b> </label>
                <label for="labels"style="margin:10px 10px 10px 453px;"> <b> Branch Code</b></label>
                <br>

                <input type="number"name="contact"value="<?php echo$row["contact"] ;?>" style="margin:10px 10px 10px 10px; height: 40px; width:47%" class="inputs">
                <input type="text" readonly name="br_code"style="margin:10px 10px 10px 10px; height: 40px; width:47%" class="inputs" value="<?php
                echo$row["br_code"] ;
                ?>">
                <br>
                <br>
                <button type="submit" name="update" style="background-color:rgba(0,123,255,255); height:40px; width:120px; border=0px;margin-left: 395px; margin-bottom:10px;">Save</button>
